...
from controllers.utils.coordinate_utils import minus, norm, polar_angle, rotate, normalize_speed
        ...
        while self.robot.step(TIME_STEP) != -1:
            ...
            if self.mode == AUTOMATIC:
                if self.current_checkpoint < len(self.path):
                    ...
                    logging.info(f"Drug deliver to {self.path[self.current_checkpoint-1]} after "
                                 f"{round(self.time/60,2)} min")
                    return
            else:
                self.manual_control()
